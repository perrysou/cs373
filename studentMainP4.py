...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER=None):
    ...
    if not OTHER:
        x = matrix(
            [[target_measurement[0]], [target_measurement[1]], [target.heading + target.turning], [target.turning]])
    ...
    u = ((hunter_position[0] - updated_measurement[0]) * (target_prediction[0] - updated_measurement[0]) + (
                hunter_position[1] - updated_measurement[1]) * (target_prediction[1] - updated_measurement[1])) / d
...
def naive_next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER):
    ...
    if not OTHER:  # first time calling this function, set up my OTHER variables.
        ...
        target_turnings = [0.0]
        OTHER = (measurements, hunter_positions, hunter_headings, target_distances,
                 target_turnings)  # now I can keep track of history
    else:  # not the first time, update my history
        target_prediction, d, theta = estimate_next_pos(target_prediction, OTHER)
        print d, theta
        u = ((hunter_position[0] - target_measurement[0]) * (target_prediction[0] - target_measurement[0]) + (
                    hunter_position[1] - target_measurement[1]) * (target_prediction[1] - target_measurement[1])) / d
        if u < 0:
            target_prediction, d, theta = estimate_next_pos(target_prediction, OTHER)
        elif u <= 1:
            pass

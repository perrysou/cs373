...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for i in range(0, N - 1):
        for d in range(2):
            for li in range(len(data[i][0])):
                l = data[i][0][li][0] + N
                z = data[i][0][li][d + 1]
                Omega[2 * i + d][2 * i + d] += 1. / measurement_noise
                Omega[2 * i + d][2 * l + d] += -1. / measurement_noise
                Omega[2 * l + d][2 * l + d] += 1. / measurement_noise
                Omega[2 * l + d][2 * i + d] += -1. / measurement_noise
                Xi[2 * i + d][0] += -z / measurement_noise
                Xi[2 * l + d][0] += z / measurement_noise
            delta = data[i][1][d]
            Omega[2 * i + d][2 * i + d] += 1. / motion_noise
            Omega[2 * i + d][2 * i + 2 + d] += -1. / motion_noise
            Omega[2 * i + 2 + d][2 * i + 2 + d] += 1. / motion_noise
            Omega[2 * i + 2 + d][2 * i + d] += -1. / motion_noise
            Xi[2 * i + d][0] += -delta / motion_noise
            Xi[2 * i + 2 + d][0] += delta / motion_noise
    O = matrix(value=Omega)
    X = matrix(value=Xi)
    mu = O.inverse() * X

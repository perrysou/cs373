...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        C = Xi.take([0, 1], [0])
        X_prime = Xi.take(range(2, dim + 2), [0])
        O_prime = Omega.take(range(2, dim + 2), range(2, dim + 2))
        Omega = O_prime - A.transpose() * B.inverse() * A
        Xi = X_prime - A.transpose() * B.inverse() * C
